  ...
  return (
    <div ref={searchRef} className="relative w-full max-w-3xl mx-auto mb-10 z-50">
      <input
        ...
        placeholder="Search tracks, playlists, URLs..."
      ...
      {showSearchDropdown && (
        <div
          className={`absolute top-full left-0 w-full mt-3 rounded-3xl border overflow-hidden shadow-2xl backdrop-blur-xl ${
            theme === 'dark'
            ...
            <div className="max-h-[420px] overflow-y-auto hide-scrollbar">
              {searchResults.map((item, idx) => (
                <button
                  key={`${item.id}-${idx}`}
                  onClick={() => {
                    sendCommand('play_search', item.query, item.source);
                    setSearchQuery('');
                    setShowSearchDropdown(false);
                  }}
