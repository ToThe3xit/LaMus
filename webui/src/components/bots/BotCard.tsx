...
const BotCard = ({ theme, bot, onSelect }: BotCardProps) => {
  ...
  return (
    <div
      onClick={() => {
        if (!isUnavailable) onSelect(bot.id);
      }}
    ...
    >
      <div
        className={`w-24 h-24 rounded-[2.5rem] flex items-center justify-center text-sm font-black shrink-0 border shadow-inner overflow-hidden ${
          theme === 'dark'
