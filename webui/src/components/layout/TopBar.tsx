...
const TopBar = ({
  ...
  const addToHistory = (
    ...
    setSearchHistory((prev) => {
      ...
      const newItem = {
        ...
        title,
        author: source === 'network' ? 'Network' : 'Library',
  ...
  return (
    <header
      className={`h-24 flex items-center px-8 justify-between border-b shrink-0 z-[60] ${
        theme === 'dark'
    ...
    >
      <div className="flex-1 pr-4 md:pr-8">
        {currentView === 'player' ? (
          /* ── Wyszukiwarka w trybie gracza ── */
          <div className="relative w-full max-w-[160px] sm:max-w-xs md:max-w-md">
            <div
              className={`flex items-center rounded-2xl border-2 transition-all w-full overflow-hidden ${
                isSearchFocused
            ...
            >
              <button
                onClick={() =>
                  setSearchSource((s) => (s === 'network' ? 'local' : 'network'))
                }
          ...
          <div className="flex items-center gap-4 sm:gap-8 animate-in fade-in duration-500">
            ...
            <div
              className={`hidden md:flex items-center p-1 px-4 rounded-full border gap-3 ${
                theme === 'dark'
