...
* Rename application.example.yml to application.yml
  * Set your Lavalink password. (Note: External network music sources are disabled by default. Check the configuration file comments to enable them. **If you enable the external plugin for Network Sources, you will most likely need to connect a Google account. It is highly recommended to use a "burner account" to avoid potential bans. You will need to follow the authentication instructions that appear in the Lavalink container logs during startup.**)
...
### Session and Bot Visibility
If you were previously logged in but closed your browser, you might encounter issues where bots are not visible or you cannot control them. This is often caused by session verification and cookie expiration. To fix this:
1. Try logging out and logging back in via the WebUI.
2. If the issue persists, manually clear your browser cookies for the application domain and log in again.